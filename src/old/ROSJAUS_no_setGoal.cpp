...
// change to ROS style parameter input
double distance_epsilon = 0.5; // for simulator use 0.5
// change to ROS style parameter input
...
struct timeval curr;
double posX,posY,posZ,posSpeed,posx,posy,posyaw;
double X_home = 0;//change these to the proper magic number
double Y_home = 0;
void translate_to_local(double X_in, double Y_in, double X_home, double Y_home, double theta, double * X_out, double * Y_out);
void translate_to_global(double X_in, double Y_in, double X_home, double Y_home, double theta, double * X_out, double * Y_out);

//This is the offset initialized to zero for the angle calculation
double posYaw_home = 0;
double posYaw = 0;
double posYawRate =0;
ELEMENT elm_list[8];//
int list_ind=0;
...
//	playerc_position2d_t *position2d;
	...
	if (playerc_position2d_subscribe(position2d, PLAYER_OPEN_MODE))
		return -1;
	////////////////////////////////////////////////////////////////////////////////////
  	//playerc_position2d_t *position2d_vfh; //not sure what this was supposed to do, but I saw it on the forum
	////////////////////////////////////////////////////////////////////////////////////
*/	
...
/* TODO this part move to OdomHandel
		...
		if(diff >= 1.0/update_rate_Hz)  // 10 Hz playerdata update rate
		{
			...
			posx=position2d->px;//global
			posyaw=position2d->pa;
			posy=position2d->py;
			// here convert ROS (x,y,yaw) to JAUS (X,Y,Yaw)
			translate_to_local(posX, posY, X_home, Y_home ,posYaw_home, &posX, &posY);			
			posYaw = angle_add(posyaw, -(posYaw_home));
			posZ=0;
			
			posSpeed=position2d->vx;
			posYawRate=position2d->va;
			//printf("posY= %f, posX = %f, posYaw = %f\n",posY, posX, posYaw);
...
*/
	...
	if (drive == true){
		...
		if (elm_set == false){ 	//get the first waypoint
			...
			translate_to_global(elm_list[active_elm].posx, -elm_list[active_elm].posy, X_home, Y_home,posYaw_home, &X_goal, &Y_goal);
                        // TODO publish ROS goal here 
			//playerc_position2d_set_cmd_pose(position2d, X_goal, Y_goal,posYaw_home,1);
			//ROSsetGoal(X_goal, Y_goal,posYaw_home, &ac);
#if(MoveBase_ENABLED)
			ac.sendGoal( ROSsetGoal(X_goal, Y_goal,posYaw_home) );
#else
#ifdef __APPLE__
  // in mac use say
  ...
  // in linux use espeak
	system("espeak 'Set New Goal' &");
		...
		double dist= sqrt( pow((posx - X_goal),2)+pow((posy - Y_goal),2));
		//printf("Distance to goal = %f m \r\n", dist);
//		if (abs((posx - X_goal))< 0.5 && abs((posy - Y_goal))<0.5){  //these are taken w/respct to local coordinates
  ...
  // in linux use espeak
	system("espeak 'Set New Goal' &");
		...
		//playerc_position2d_set_cmd_pose(position2d, (X_goal), (Y_goal), posYaw_home,1);  // let robot can drive back and face same direction
...
void SEND_REPORT_LOCAL_POSE (long handle,unsigned int COP_id)
{
    ...
    if ( VehicleStatus== Status_READY )
	{
		...
		lpmsg.X = scaleToUInt32(posX, -100000, 100000);
		lpmsg.Y = scaleToUInt32(posY, -100000, 100000);
		//lpmsg.Z = scaleToUInt32(posZ, -100000, 100000);
		
		//lpmsg.Yaw = scaleToUInt16(posYaw, -1*PI, PI);
...
void SEND_REPORT_VELOCITY_STATE (long handle,unsigned int COP_id)
{
    ...
    if ( VehicleStatus== Status_READY )
	{
		...
		rvmsg.VX=scaleToUInt32( posSpeed, -327.68, 327.67);
		rvmsg.YawRate=scaleToUInt16(posYawRate, -32.768 , 32.767 );
...
{			printf("Message: SetLocalPose              received\r\n");
		  ...
		  float Yaw= ceil(UInt16ToScale(Yaw_hold, -1*PI, PI));
			...
			//posYaw_home = posyaw;//angle_add(posyaw, (-Yaw));
			posYaw_home = normalize(posyaw - Yaw);
			X_home = posx;// - X;
			Y_home = posy;// - Y;
			printf("SetLocalPose to X = %f, Y = %f, Yaw = %f\n", X , Y, Yaw);
			//playerc_position2d_set_odom(position2d,X,Y,posyaw); // now just set to zero
		...
		case JAUS_ID_ExecuteList              :
{
		...
		case JAUS_ID_QueryTravelSpeed         :
			...
			SEND_REPORT_TRAVEL_SPEED(handle, COP_JausID,posSpeed);
			break; 
...
double angle_add(double pa,double pa_return){
if(pa < 0)
   pa = pa + 2*PI;
	
if(pa_return <0)
    pa_return = pa_return + 2*PI;

double angle = pa + pa_return;
...
void translate_to_local(double X_in, double Y_in, double X_home, double Y_home, double theta, double * X_out, double * Y_out)
{
	double theta_to_local = atan2(-Y_home, -X_home);
	//theta_to_local = anle_add(posYaw_home, -theta_to_local);
	theta_to_local = theta_to_local - posYaw_home;
	//theta_to_local = no
	double d = sqrt(pow(X_home,2) + pow(Y_home,2));
	double X_back = d*cos(theta_to_local);
	double Y_back = d*sin(theta_to_local);
	
	
	*X_out = cos(theta)*X_in + sin(theta)*Y_in + X_back;
	*Y_out = (-sin(theta)*X_in +cos(theta)*Y_in) + Y_back;
		/*if(theta > PI/2) {
		*X_out = -*X_out;}
		if( theta < 0)
		*Y_out = -*Y_out;
...
void translate_to_global(double X_in, double Y_in, double X_home, double Y_home, double theta, double * X_out, double * Y_out)
{
	*X_out = cos(theta)*X_in - sin(theta)*Y_in + X_home;
	*Y_out = (sin(theta)*X_in + cos(theta)*Y_in) + Y_home;
	/*if(theta > PI/2) {
		*X_out = -*X_out;}
		if( theta < 0)
		*Y_out = -*Y_out;
...
*/
  ...
  posyaw = tf::getYaw(pose.getRotation());

  translate_to_local(posX, posY, X_home, Y_home ,posYaw_home, &posX, &posY);			
  posYaw = angle_add(posyaw, -(posYaw_home));
  posZ=0;
  ROS_INFO("ROS:[%5.2f,%5.2f,%5.2f],JAUS:[%5.2f,%5.2f,%5.2f]", posx,posy,posyaw,posX,posY,posYaw);
